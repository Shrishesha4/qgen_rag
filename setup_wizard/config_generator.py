...
def generate_docker_compose(
    ...
    if services_enabled.get("db", True):
        lines.extend(["  postgres_data:", "    name: qgen_postgres_data", ""])
    if services_enabled.get("redis", True):
        lines.extend(["  redis_data:", "    name: qgen_redis_data", ""])
    if services_enabled.get("api", True):
        lines.extend([
            "  upload_data:", "    name: qgen_upload_data", "",
            "  model_cache:", "    name: qgen_model_cache", "",
        ])
    if services_enabled.get("ollama", False):
        lines.extend(["  ollama_data:", "    name: qgen_ollama_data", ""])
