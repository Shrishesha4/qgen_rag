...
async def get_questions_for_vetting(
    ...
    # Teachers are scoped to questions they can legitimately vet:
    # - Questions under subjects they own
    # - Questions generated by their own generation sessions (shared-subject flows)
    if current_user.role == "teacher" and not teacher_id:
        query = query.outerjoin(
            GenerationSession,
            GenerationSession.id == Question.session_id,
        ).where(
            or_(
                Question.subject_id.in_(
                    select(Subject.id).where(Subject.user_id == current_user.id)
