<script lang="ts">
	...
	async function approve() {
		...
		submitting = true;
		try {
			if (currentQuestion.difficulty_level !== undefined && currentQuestion.difficulty_level !== null) {
				// preserve current difficulty when approving without a tile selection
			}
			await submitVetting({
				question_id: currentQuestion.id,
				decision: 'approve',
	...
	async function submitRejection() {
		...
		try {
			...
			if (result.improved && result.improved_text) {
				// Update the current question in-place with improved version
